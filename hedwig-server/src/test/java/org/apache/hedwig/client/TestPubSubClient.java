...
public class TestPubSubClient extends PubSubServerStandAloneTestBase {
    ...
    class TestSubscriptionListener implements SubscriptionListener {
        ...
        @Override
        public void processEvent(final ByteString topic, final ByteString subscriberId, final SubscriptionEvent event) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    logger.debug("Event {} received for subscription(topic:{}, subscriber:{})", new Object[] { event,
    ...
    public void testSyncPublish() throws Exception {
        ...
        try {
            publisher.publish(ByteString.copyFromUtf8("mySyncTopic"),
                    Message.newBuilder().setBody(ByteString.copyFromUtf8("Hello Sync World!")).build());
        } catch (Exception e) {
    ...
    public void testSyncPublishWithResponse() throws Exception {
        // ByteString topic =
        // ByteString.copyFromUtf8("testSyncPublishWithResponse");
        ByteString topic = ByteString.copyFromUtf8("test1");
        ...
        SubscriptionOptions opts = SubscriptionOptions.newBuilder().setCreateOrAttach(CreateOrAttach.CREATE_OR_ATTACH)
                .build();
    ...
    public void testAsyncPublish() throws Exception {
        publisher.asyncPublish(ByteString.copyFromUtf8("myAsyncTopic"),
                Message.newBuilder().setBody(ByteString.copyFromUtf8("Hello Async World!")).build(),
    ...
    public void testMultipleAsyncPublish() throws Exception {
        ...
        assertTrue(queue.take());
        publisher.asyncPublish(topic1,
                Message.newBuilder().setBody(ByteString.copyFromUtf8("Hello Again on old topic!")).build(),
                new TestCallback(), null);
        assertTrue(queue.take());
    }
    ...
    public void testAsyncSubscribe() throws Exception {
        SubscriptionOptions opts = SubscriptionOptions.newBuilder().setCreateOrAttach(CreateOrAttach.CREATE_OR_ATTACH)
                .build();
        subscriber.asyncSubscribe(ByteString.copyFromUtf8("myAsyncSubscribeTopic"), ByteString.copyFromUtf8("1"), opts,
                new TestCallback(), null);
        assertTrue(queue.take());
    }
    ...
    public void testSubClosesubAndPublish() throws Exception {
        ...
        subscriber.startDelivery(topic, subid, new TestMessageHandler());
        for (int i = 0; i < 3; i++) {
            publisher.asyncPublish(topic, Message.newBuilder().setBody(ByteString.copyFromUtf8("Message #" + i))
                    .build(), new TestCallback(), null);
            assertTrue(queue.take());
            assertTrue(consumeQueue.take());
        }
    ...
    public void testSyncSubscribeWithListener() throws Exception {
        ...
        try {
            SubscriptionOptions options = SubscriptionOptions.newBuilder()
                    .setCreateOrAttach(CreateOrAttach.CREATE_OR_ATTACH).setEnableResubscribe(false).build();
    ...
    public void testSyncSubscribeForceAttach() throws Exception {
        ...
        subscriber.addSubscriptionListener(new TestSubscriptionListener());
        SubscriptionOptions options = SubscriptionOptions.newBuilder()
                .setCreateOrAttach(CreateOrAttach.CREATE_OR_ATTACH).setForceAttach(true).setEnableResubscribe(false)
                .build();
        try {
            subscriber.subscribe(topic, subscriberId, options);
        } catch (PubSubException.ServiceDownException e) {
    ...
    public void testSyncSubscribeWithListenerWhenReleasingTopic() throws Exception {
        ...
        subscriber.addSubscriptionListener(new TestSubscriptionListener());
        SubscriptionOptions options = SubscriptionOptions.newBuilder()
                .setCreateOrAttach(CreateOrAttach.CREATE_OR_ATTACH).setForceAttach(false).setEnableResubscribe(false)
                .build();
        try {
            subscriber.subscribe(topic, subscriberId, options);
        } catch (PubSubException.ServiceDownException e) {
    ...
    public void testCloseSubscribeDuringResubscribe() throws Exception {
        ...
        subscriber.addSubscriptionListener(new TestSubscriptionListener());
        SubscriptionOptions options = SubscriptionOptions.newBuilder()
                .setCreateOrAttach(CreateOrAttach.CREATE_OR_ATTACH).setForceAttach(false).setEnableResubscribe(true)
                .build();
        subscriber.subscribe(topic, subscriberId, options);
