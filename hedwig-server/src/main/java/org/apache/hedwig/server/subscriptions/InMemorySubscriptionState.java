...
public class InMemorySubscriptionState {
    ...
    public InMemorySubscriptionState(SubscriptionData subscriptionData, Version version, MessageSeqId lastConsumeSeqId) {
        ...
        if (subscriptionData.hasPreferences()) {
            ...
            // progate the old system preferences from subscription state to
            // preferences
    ...
    public SubscriptionData toSubscriptionData() {
        SubscriptionState.Builder stateBuilder = SubscriptionState.newBuilder(subscriptionState).setMsgId(
                lastConsumeSeqId);
        return SubscriptionData.newBuilder().setState(stateBuilder).setPreferences(subscriptionPreferences).build();
    }
     ...
     * Update preferences.
     * 
     * @return true if preferences is updated, which needs to be persisted,
     *         false otherwise.
     */
    public boolean updatePreferences(SubscriptionPreferences preferences) {
        boolean changed = false;
        SubscriptionPreferences.Builder newPreferencesBuilder = SubscriptionPreferences
                .newBuilder(subscriptionPreferences);
        ...
        if (preferences.hasMessageWindowSize()) {
            if (!subscriptionPreferences.hasMessageWindowSize()
                    || subscriptionPreferences.getMessageWindowSize() != preferences.getMessageWindowSize()) {
