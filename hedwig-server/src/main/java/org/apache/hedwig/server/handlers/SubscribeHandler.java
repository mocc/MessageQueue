...
public class SubscribeHandler extends BaseHandler {
    ...
    public SubscribeHandler(ServerConfiguration cfg, TopicManager topicMgr, DeliveryManager deliveryManager,
            PersistenceManager persistenceMgr, SubscriptionManager subMgr, SubscriptionChannelManager subChannelMgr) {
        super(topicMgr, cfg);
    ...
    public void handleRequestAtOwner(final PubSubRequest request, final Channel channel) {
        ...
        // delete for MQ sematics

        // MessageSeqId lastSeqIdPublished =
        // MessageSeqId.newBuilder(seqId).setLocalComponent(seqId.getLocalComponent()).build();

        final long requestTime = MathUtils.now();
        subMgr.serveSubscribeRequest(topic, subRequest, consumeSeqId, new Callback<SubscriptionData>() {
            ...
            public void operationFinished(Object ctx, final SubscriptionData subData) {
                ...
                deliveryMgr.startServingSubscription(topic, subscriberId, subData.getPreferences(), seqIdToStartFrom,
                        new ChannelEndPoint(channel), filter, new Callback<Void>() {
                            @Override
                            public void operationFinished(Object ctx, Void result) {
                                // First write success and then tell the
                                // delivery manager,
                                // otherwise the first message might go out
                                // before the response
                                // to the subscribe
