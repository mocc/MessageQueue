...
public class BookkeeperPersistenceManager implements PersistenceManagerWithRangeScan, TopicOwnershipChangeListener {
    ...
    static class TopicInfo {
         ...
         * add for MQ sematics
         * 
         * first message-seq-id that should be delivered for queue receiver
         * 
         */
        MessageSeqId firstSeqIdToDeliver;
     ...
     *            A executor
     */
    public BookkeeperPersistenceManager(BookKeeper bk, MetadataManagerFactory metaManagerFactory, TopicManager tm,
            ServerConfiguration cfg, ScheduledExecutorService executor) {
        this.bk = bk;
    ...
    private static LedgerRange buildLedgerRange(long ledgerId, long startOfLedger, MessageSeqId endOfLedger) {
        LedgerRange.Builder builder = LedgerRange.newBuilder().setLedgerId(ledgerId)
    ...
    class RangeScanOp extends TopicOpQueuer.SynchronousOp {
        ...
        protected void runInternal() {
            ...
            // if startSeqIdToScan is less than zero, which means it is an
            // unfinished scan request
        ...
        protected void read(final InMemoryLedgerRange imlr, final long startSeqId, final long endSeqId) {
            ...
            if (imlr.getStartSeqIdIncluded() > startSeqId) {
                ...
                request.callback.scanFailed(request.ctx, new PubSubException.UnexpectedConditionException(
                        "Scan request is out of range"));
            ...
            imlr.handle.asyncReadEntries(startSeqId - imlr.getStartSeqIdIncluded(),
                    correctedEndSeqId - imlr.getStartSeqIdIncluded(), new SafeAsynBKCallback.ReadCallback() {
                        ...
                        public void safeReadComplete(int rc, LedgerHandle lh, Enumeration<LedgerEntry> seq, Object ctx) {
                            if (rc != BKException.Code.OK || !seq.hasMoreElements()) {
                                if (rc == BKException.Code.OK) {
                                    // means that there is no entries read,
                                    // provide a meaningful exception
                                    rc = BKException.Code.NoSuchEntryException;
                                }
                                BKException bke = BKException.create(rc);
                                logger.error("Error while reading from ledger: " + imlr.range.getLedgerId()
                                        + " for topic: " + topic.toStringUtf8(), bke);
                            ...
                            while (seq.hasMoreElements()) {
                                ...
                                try {
                                    ...
                                    logger.error(msg, e);
                                    request.callback.scanFailed(ctx, new PubSubException.UnexpectedConditionException(
                                            msg));
                                    return;
                                }

                                logger.debug("Read response from ledger: {} entry-id: {}", lh.getId(),
                                        entry.getEntryId());
                            ...
                            scanMessages(request, imlr.getStartSeqIdIncluded() + entry.getEntryId() + 1,
                                    numMessagesRead, totalSizeRead);
                        }
                    }, request.ctx);
        }
    ...
    class UpdateLedgerOp extends TopicOpQueuer.AsynchronousOp<Void> {
        ...
        public UpdateLedgerOp(ByteString topic, final Callback<Void> cb, final Object ctx, Set<Long> ledgersDeleted) {
        ...
        public void run() {
            ...
            for (Map.Entry<Long, InMemoryLedgerRange> e : topicInfo.ledgerRanges.entrySet()) {
                ...
                if (!foundUnconsumedLedger && ledgersDeleted.contains(ledgerId)) {
                    ...
                    if (!lr.hasEndSeqIdIncluded()) {
                        String msg = "Should not remove unclosed ledger " + ledgerId + " for topic "
                                + topic.toStringUtf8();
                        logger.error(msg);
            ...
            if (!keysToRemove.isEmpty()) {
                ...
                tpManager.writeTopicPersistenceInfo(topic, newRanges, topicInfo.ledgerRangesVersion,
        ...
        public void runInternal() {
            ...
            for (Long endSeqIdIncluded : topicInfo.ledgerRanges.keySet()) {
                if (endSeqIdIncluded <= seqId) {
                    // This ledger's message entries have all been consumed
                    // already
    ...
    private void deleteLedgersAndUpdateLedgersRange(final ByteString topic, final LinkedList<Long> ledgersToDelete,
        ...
        bk.asyncDeleteLedger(ledger, new DeleteCallback() {
            ...
            public void deleteComplete(int rc, Object ctx) {
                if (BKException.Code.NoSuchLedgerExistsException == rc || BKException.Code.OK == rc) {
    ...
    public MessageSeqId getLastSeqIdReceived(ByteString topic) throws ServerNotResponsibleForTopicException {
        ...
        long currentSeqId = topicInfo.firstSeqIdToDeliver.getLocalComponent() - 1;
        return MessageSeqId.newBuilder().setLocalComponent(currentSeqId).build();
    }
    ...
    protected void releaseTopicIfRequested(final ByteString topic, Exception e, Object ctx) {
        ...
        if (topicInfo.doRelease.compareAndSet(false, true)) {
            logger.info("Release topic " + topic.toStringUtf8()
                    + " when bookkeeper persistence mananger encounters failure :", e);
            tm.releaseTopic(topic, new Callback<Void>() {
                ...
                public void operationFinished(Object ctx, Void resultOfOperation) {
                    logger.info("successfully releasing topic {} when encountering" + " exception from bookkeeper",
                            topic.toStringUtf8());
                }
            }, null);
        }
    ...
    protected void doPersistMessage(final PersistRequest request) {
        ...
        if (topicInfo.doRelease.get()) {
            request.getCallback().operationFailed(
                    request.ctx,
        ...
        // if the topic is changing ledger, queue following persist requests
        // until ledger is changed
        if (topicInfo.doChangeLedger.get()) {
            logger.info("Topic {} is changing ledger, so queue persist request for message.", topic.toStringUtf8());
        ...
        if (UNLIMITED_ENTRIES != maxEntriesPerLedger && entriesInThisLedger >= maxEntriesPerLedger) {
            if (topicInfo.doChangeLedger.compareAndSet(false, true)) {
                // for order guarantees, we should wait until all the adding
                // operations for current ledger
                // are succeed. so we just mark it as
                // lastSeqIdBeforeLedgerChange
                // when the lastSeqIdBeforeLedgerChange acked, we do changing
                // the ledger
        ...
        topicInfo.currentLedgerRange.handle.asyncAddEntry(msgToSerialize.toByteArray(),
                new SafeAsynBKCallback.AddCallback() {
                    ...
                    public void safeAddComplete(int rc, LedgerHandle lh, long entryId, Object ctx) {
                        ...
                        // avoid double callback by mistake, since we may do
                        // change ledger in this callback.
                        ...
                        // if this acked entry is the last entry of current
                        // ledger
                        // we can add a ChangeLedgerOp to execute to change
                        // ledger
                        if (topicInfo.doChangeLedger.get()
                            ...
                            changeLedger(topic, new Callback<Void>() {
                                ...
                                public void operationFailed(Object ctx, PubSubException exception) {
                                    ...
                                    // change ledger failed, we should give up
                                    // topic
                                ...
                                public void operationFinished(Object ctx, Void resultOfOperation) {
                                    ...
                                    topicInfo.lastSeqIdBeforeLedgerChange = -1;
                                    // the ledger is changed, persist queued
                                    // requests
                                    // if the number of queued persist requests
                                    // is more than maxEntriesPerLedger
                                    // we just persist maxEntriesPerLedger
                                    // requests, other requests are still queued
                                    // until next ledger changed.
                                    int numRequests = 0;
                                    while (!topicInfo.deferredRequests.isEmpty() && numRequests < maxEntriesPerLedger) {
                                        PersistRequest pr = topicInfo.deferredRequests.removeFirst();
                                        doPersistMessage(pr);
                                        ++numRequests;
                                    }
                                    logger.debug(
                                            "Finished persisting {} queued requests, but there are still {} requests in queue.",
    ...
    class AcquireOp extends TopicOpQueuer.AsynchronousOp<Void> {
         ...
         */
        void getStartSeqIdToProcessTopicLedgerRanges(final List<LedgerRange> rangesList, final Version version) {
            ...
            bk.asyncOpenLedger(ledgerId, DigestType.CRC32, passwd, new SafeAsynBKCallback.OpenCallback() {
                ...
                public void safeOpenComplete(int rc, LedgerHandle ledgerHandle, Object ctx) {
                    ...
                    // the ledger is closed before, calling close is just a nop
                    // operation.
                    try {
                        ledgerHandle.close();
                    } catch (InterruptedException ie) {
                        // the exception would never be thrown for a read only
                        // ledger handle.
        ...
        void processTopicLedgerRanges(final List<LedgerRange> rangesList, final Version version, long startOfLedger) {
            ...
            while (lrIterator.hasNext()) {
                ...
                if (range.hasEndSeqIdIncluded()) {
                    ...
                    if (range.hasStartSeqIdIncluded()) {
                        ...
                        range = buildLedgerRange(range.getLedgerId(), startOfLedger, range.getEndSeqIdIncluded());
                    }
                ...
                // find it out
                recoverLastTopicLedgerAndOpenNewOne(range.getLedgerId(), startOfLedger, version, topicInfo);
                return;
            }
            ...
            bk.asyncOpenLedger(ledgerId, DigestType.CRC32, passwd, new SafeAsynBKCallback.OpenCallback() {
                ...
                public void safeOpenComplete(int rc, LedgerHandle ledgerHandle, Object ctx) {
                    ...
                    if (numEntriesInLastLedger <= 0) {
                        ...
                        openNewTopicLedger(topic, expectedVersionOfLedgerNode, topicInfo, expectedStartSeqId, false,
                                cb, ctx);
                        return;
                    }
                    ...
                    ledgerHandle.asyncReadEntries(numEntriesInLastLedger - 1, numEntriesInLastLedger - 1,
                            new SafeAsynBKCallback.ReadCallback() {
                                ...
                                public void safeReadComplete(int rc, LedgerHandle lh, Enumeration<LedgerEntry> seq,
                                    ...
                                    if (rc != BKException.Code.OK || !seq.hasMoreElements()) {
                                        if (rc == BKException.Code.OK) {
                                            // means that there is no entries
                                            // read, provide a meaningful
                                            // exception
                                            rc = BKException.Code.NoSuchEntryException;
                                        }
                                        ...
                                        BKException bke = BKException.create(rc);
                                        logger.error(
                                                "While recovering ledger: " + ledgerId + " for topic: "
                                    ...
                                    if (startOfLedger != expectedStartSeqId) {
                                        // gap would be introduced by old
                                        // version when gc consumed ledgers
                                        String msg = "Expected start seq id of recovered ledger " + ledgerId
                                                + " to be " + expectedStartSeqId + " but it was " + startOfLedger + ".";
                                        logger.warn(msg);
                                    }

                                    LedgerRange lr = buildLedgerRange(ledgerId, startOfLedger, lastMessage.getMsgId());
                                    topicInfo.ledgerRanges.put(endOfLedger, new InMemoryLedgerRange(lr, lh));

                                    logger.info(
                                            "Recovered unclosed ledger: {} for topic: {} with {} entries starting from seq id {}",
                                            va(ledgerId, topic.toStringUtf8(), numEntriesInLastLedger, startOfLedger));

                                    openNewTopicLedger(topic, expectedVersionOfLedgerNode, topicInfo, endOfLedger + 1,
                                            false, cb, ctx);
                                }
                            }, ctx);
     ...
     *            Callback context.
     */
    void openNewTopicLedger(final ByteString topic, final Version expectedVersionOfLedgersNode,
            final TopicInfo topicInfo, final long startSeqId, final boolean changeLedger, final Callback<Void> cb,
            final Object ctx) {
        bk.asyncCreateLedger(cfg.getBkEnsembleSize(), cfg.getBkWriteQuorumSize(), cfg.getBkAckQuorumSize(),
                DigestType.CRC32, passwd, new SafeAsynBKCallback.CreateCallback() {
                    AtomicBoolean processed = new AtomicBoolean(false);
                    ...
                    public void safeCreateComplete(int rc, LedgerHandle lh, Object ctx) {
                        ...
                        if (rc != BKException.Code.OK) {
                            BKException bke = BKException.create(rc);
                            logger.error("Could not create new ledger while acquiring topic: " + topic.toStringUtf8(),
                                    bke);
                            cb.operationFailed(ctx, new PubSubException.ServiceDownException(bke));
                            return;
                        }
                        ...
                        // compute last seq id
                        // compute oldes seq id
                        if (!changeLedger) {
                            topicInfo.lastSeqIdPushed = topicInfo.ledgerRanges.isEmpty() ? MessageSeqId.newBuilder()
                                    .setLocalComponent(startSeqId - 1).build() : topicInfo.ledgerRanges.lastEntry()
                                    .getValue().range.getEndSeqIdIncluded();

                            // add for MQ sematics
                            topicInfo.firstSeqIdToDeliver = topicInfo.ledgerRanges.isEmpty() ? MessageSeqId
                                    .newBuilder().setLocalComponent(START_SEQ_ID).build() : MessageSeqId
                                    .newBuilder()
                                    .setLocalComponent(
                                            topicInfo.ledgerRanges.firstEntry().getValue().getStartSeqIdIncluded())
                                    .build();
                        }
                        ...
                        tpManager.writeTopicPersistenceInfo(topic, builder.build(), expectedVersionOfLedgersNode,
                                new Callback<Version>() {
                                    @Override
                                    public void operationFinished(Object ctx, Version newVersion) {
                                        // Finally, all done
            ...
            topicInfo.currentLedgerRange.handle.asyncClose(new CloseCallback() {
                ...
                public void closeComplete(int rc, LedgerHandle lh, Object ctx) {
                    ...
                    if (BKException.Code.OK != rc) {
                        BKException bke = BKException.create(rc);
                        logger.error(
                                "Could not close ledger " + ledgerId + " while changing ledger of topic "
                                        + topic.toStringUtf8(), bke);
                        cb.operationFailed(ctx, new PubSubException.ServiceDownException(bke));
                        return;
                    }
                    ...
                    // update last range
                    LedgerRange lastRange = buildLedgerRange(ledgerId,
                            topicInfo.currentLedgerRange.getStartSeqIdIncluded(), topicInfo.lastSeqIdPushed);
                    ...
                    logger.info("Closed written ledger " + ledgerId + " for topic " + topic.toStringUtf8()
                            + " to change ledger.");
                    openNewTopicLedger(topic, topicInfo.ledgerRangesVersion, topicInfo, endSeqId + 1, true, cb, ctx);
                }
            }, ctx);
        }
